...
Layers: `core` is the shared pipeline; `maf`, `langchain`, `langgraph` and `crew`
are the four framework adapters. A concept bound only in `core` is shared verbatim
by all four frameworks.
...
| Agent loop: Action | `core` | `onboarding.core.mailer:build_already_registered_mail` |
...
| Agent loop: Action | `core` | `onboarding.core.steps:notify_already_registered` |
...
| Agent loop: Action | `core` | `onboarding.core.tasks:summarise` |
| Agent loop: Action | `core` | `onboarding.core.tasks:write_tasks` |
| Agent loop: Action | `crew` | `onboarding.adapters.crew.tools:check_region_compliance` |
| Agent loop: Action | `crew` | `onboarding.adapters.crew.tools:lookup_product_catalog` |
| Agent loop: Action | `crew` | `onboarding.adapters.crew.tools:lookup_sla_matrix` |
...
| Agent loop: Reflection | `crew` | `onboarding.adapters.crew.tools:check_business_rules` |
...
| Workflow decomposition | `crew` | `onboarding.adapters.crew.crew:build_crew` |
...
| Context-aware prompt enhancement | `crew` | `onboarding.adapters.crew.tools:get_customer_context` |
...
| PII detection & masking | `crew` | `onboarding.adapters.crew.tools:get_customer_context` |
...
| No fabricated discounts / grounded claims | `crew` | `onboarding.adapters.crew.tools:check_business_rules` |
...
| Tone policy | `crew` | `onboarding.adapters.crew.tools:check_business_rules` |
| Tone policy | `crew` | `onboarding.adapters.crew.tools:get_email_length_policy` |

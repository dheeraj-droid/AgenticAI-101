"""The headline claim: the same record produces the same decisions in all three.

Model-free. Every fixture used here reaches a terminal state without drafting —
blocked, escalated or rejected — so the deterministic half of the pipeline is
compared exactly, with no endpoint required.
"""
...
MODEL_FREE_FIXTURES = ["enterprise_high_value", "invalid_missing_fields", "injection_attempt"]
...
async def test_status_and_risk_agree(fixture, record, record_path) -> None:
    ...
    assert len({r.risk.requires_human_approval for r in results.values()}) == 1
...
async def test_injection_record_is_routed_to_a_human(record, record_path) -> None:
    """An injection attempt must never be handled autonomously."""
    ...
    for framework, result in results.items():
        ...
        assert result.status == "blocked_awaiting_approval", f"{framework} did not stop for a human"
